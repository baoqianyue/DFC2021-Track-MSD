...
NLCD_IDX_TO_REDUCED_LC_ACCUMULATOR = np.array([
    [0, 0, 0, 0, 1],  # 0 No data 0 0
    [1, 0, 0, 0, 0],  # 1 Open Water 11
    [0, 0, 0, 0, 1],  # 2 Ice/Snow 12
    [0, 0, 0, 0, 0],  # 3 Developed Open Space 21
    [0, 0, 0, 0, 0],  # 4 Developed Low Intensity 22
    [0, 0, 0, 1, 0],  # 5 Developed Medium Intensity 23
    [0, 0, 0, 1, 0],  # 6 Developed High Intensity 24
    [0, 0, 0, 0, 0],  # 7 Barren Land 31
    [0, 1, 0, 0, 0],  # 8 Deciduous Forest 41 Tree
    [0, 1, 0, 0, 0],  # 9 Evergreen Forest 42 Tree
    [0, 1, 0, 0, 0],  # 10 Mixed Forest 43 Tree
    [0, 1, 0, 0, 0],  # 11 Shrub/Scrub 52 Tree
    [0, 0, 1, 0, 0],  # 12 Grassland/Herbaceous 71
    [0, 0, 1, 0, 0],  # 13 Pasture/Hay 81
    [0, 0, 1, 0, 0],  # 14 Cultivated Crops 82
    [0, 1, 0, 0, 0],  # 15 Woody Wetlands 90 Tree
    [0, 1, 0, 0, 0],  # 16 Emergent Herbaceious Wetlands 95 Tree
])
...
def fit(model, device, data_loader, num_batches, optimizer, criterion, epoch, memo=''):
    ...
    for batch_idx, (data, targets) in tqdm(enumerate(data_loader), total=num_batches, file=sys.stdout):
        ...
        optimizer.zero_grad()
        outputs = model(data)
        # print('outputs shape: {}, targets shape: {}'.format(outputs.shape, targets.shape))
