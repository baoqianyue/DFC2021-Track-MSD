...
from dataloaders.Landsat2NlcdDatasets import Landsat2NlcdDataset
...
LANDSAT_2013_MEANS = np.array([1658.13, 2555.54, 376.24])
LANDSAT_2013_STDS = np.array([793.06, 1159.92, 154.90])
...
def image_transforms(img, group):
    if group == 0:
        img = (img - LANDSAT_2013_MEANS) / LANDSAT_2013_STDS
    elif group == 1:
...
def main():
    ...
    # criterion = nn.CrossEntropyLoss(weight=weight_clean_fcn_label, size_average=True)
