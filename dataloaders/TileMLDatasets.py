...
class TileMLTrainDataset(Dataset):

    def __init__(self, fn, label_fn, chip_size, stride, image_transform=None, label_transform=None,
                 windowed_sampling=False, verbose=False):
        ...
        with rasterio.open(self.label_fn) as lf:
            if not windowed_sampling:
                self.label = np.rollaxis(lf.read(), 0, 3)
    ...
    def __getitem__(self, idx):
        ...
        if self.windowed_sampling:
            ...
            # print('self.label shape: {}'.format(self.label.shape))

            img = self.data[y:y + self.chip_size, x:x + self.chip_size]
            label = self.label[y:y + self.chip_size, x:x + self.chip_size]
