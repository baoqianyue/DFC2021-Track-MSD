    ...
    def stream_tile_fns(self):
        ...
        for idx in range(lower_idx, upper_idx):
            ...
            if self.use_labels:
                ...
                # print('img_fn: {}, label_fn: {}'.format(img_fn, label_fn))
                img_fn = re.findall(r'\bdata.*\b', img_fn)[0]
                img_fn = img_fn.split('/')
                img_fn = os.path.join('./data/image', img_fn[1], img_fn[2])
                ...
                # if origin == 1:
                #     ################################################ origin label
                #     label_fn = os.path.join('./data/image', label_fn[1],
                #                             label_fn[2])
                # else:
                #     ################################################ 验证集伪标签
                #     year = label_fn[1]
                #     label_fn = label_fn[2]
                #     label_fn = label_fn.replace('nlcd', 'predictions')
                #     if year == 'nlcd-2016':
                #         label_fn = label_fn.replace('2016', '2017')
                #     label_fn = os.path.join('./data/image/t_deploy_56ed_process2', label_fn)
                ...
                if year == 'nlcd-2016':
                    label_fn = label_fn.replace('2016', '2017')
                label_fn = os.path.join('./data/image/t_deploy_56ed_process4', label_fn)
                ...
                # if year == 'nlcd-2016':
                #     label_fn = label_fn.replace('2016', '2017')
                # label_fn = os.path.join('./data/image/59_t_deploy_56ed', label_fn)

                ################################################ fcn_label
                # label_fn_2 = label_fn[2]
                # if label_fn_2.endswith('2016.tif'):
                #     label_fn_2 = label_fn_2.replace('2016', '2017')
                # label_fn = os.path.join('./data/60_label', label_fn[1],
                #                         label_fn_2)

                ################################################ landsat_label
                # label_fn_1 = label_fn[1]
                # label_fn_2 = label_fn[2]
                # if label_fn_2.endswith('2016.tif'):
                #     label_fn_1 = label_fn_1.replace('2016', '2017')
                #     label_fn_2 = label_fn_2.replace('2016', '2017')
                # label_fn = os.path.join('./data/landsat_nlcd', label_fn_1,
                #                         label_fn_2)
            else:
                img_fn = self.fns[idx]
                img_fn = re.findall(r'\bdata.*\b', img_fn)[0]
    ...
    def stream_chips(self):
        for img_fn, label_fn, group in self.stream_tile_fns():
            ...
            for i in range(self.num_chips_per_tile):
                ...
                # print('num_chips_per_tile len: {}'.format(self.num_chips_per_tile))

                # print('{}th, width range: {}'.format(i, width - self.chip_size))
                # print('{}th, height range: {}'.format(i, height - self.chip_size))
                x = np.random.randint(0, width - self.chip_size)
                y = np.random.randint(0, height - self.chip_size)

                # print('{}th, x: {}, y: {}'.format(i, x, y))
