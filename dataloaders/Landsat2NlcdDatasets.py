...
class Landsat2NlcdDataset(IterableDataset):
    ...
    def stream_tile_fns(self):
        ...
        for idx in range(lower_idx, upper_idx):
            ...
            if self.use_labels:
                ...
                img_fn1 = img_fn[1]
                img_fn2 = img_fn[2]
                if img_fn1 == 'naip-2017':
                    img_fn1 = img_fn1.replace('2017', '2016')
                    img_fn2 = img_fn2.replace('2017', '2016')
                img_fn1 = img_fn1.replace('naip', 'landsat')
                img_fn2 = img_fn2.replace('naip', 'landsat')
                img_fn = os.path.join('./data/image', img_fn1, img_fn2)
                # print('img_fn: {}'.format(img_fn))
                label_fn = re.findall(r'\bdata.*\b', label_fn)[0]
                label_fn = label_fn.split('/')
                ################################################ origin label
                label_fn = os.path.join('./data/image', label_fn[1],
                                        label_fn[2])
                # print('label_fn: {}'.format(label_fn))
        ...
        for img_fn, label_fn, group in self.stream_tile_fns():
            ...
            try:
                if not self.windowed_sampling:
                    img_data = np.rollaxis(img_tmp, 0, 3)
                    if self.use_labels:
