...
def main():
    ...
    for image_idx in range(len(image_fns)):
        ...
        single_output = np.zeros((2, input_height, input_width), dtype=np.float32)
        kernel = np.ones((CHIP_SIZE, CHIP_SIZE), dtype=np.float32)
        single_kernel = np.ones((CHIP_SIZE, CHIP_SIZE), dtype=np.float32)
        kernel[HALF_PADDING:-HALF_PADDING, HALF_PADDING:-HALF_PADDING] = 5
        single_kernel[HALF_PADDING:-HALF_PADDING, HALF_PADDING:-HALF_PADDING] = 5
        counts = np.zeros((input_height, input_width), dtype=np.float32)
        single_counts = np.zeros((input_height, input_width), dtype=np.float32)
        ...
        for i, (data, coords) in enumerate(dataloader):
            ...
            with torch.no_grad():
                ...
                single_data = data[:, 3, :, :].unsqueeze(1)  # expand one shape for one channel naip
                single_t_output = single_model(single_data)
                single_t_output = F.softmax(single_t_output, dim=1).cpu().numpy()
            ...
            for j in range(t_output.shape[0]):
                ...
                single_output[:, y:y + CHIP_SIZE, x:x + CHIP_SIZE] += single_t_output[j] * single_kernel
                single_counts[y:y + CHIP_SIZE, x:x + CHIP_SIZE] += single_kernel

        output = output / counts
        output_hard = output.argmax(axis=0).astype(np.uint8)

        single_output = single_output / single_counts
        single_output_hard = single_output.argmax(axis=0).astype(np.uint8)
        indexes = single_output_hard == 1
        output_hard[indexes == True] = 1
